...
public:
  ...
  static Series<T> fromArray(std::vector<uint16_t> shape,
                             std::vector<T> input) {
  ...
  /** @brief The only use of this class is for auto instancing these three
   * default values to a standard. Then call subclass constructor to create the
   * real values
   *
  ...
  /** @brief Loads series data from text files. Assumes data are formatted as
   * rows, where each record is a row of numbers separated by spaces e.g. 'v v v
   * v v'. You can optionally specify a fixed number of initial items per row to
   * skip / discard.
   *
   *
   */
   ...
   */
  Series<T> max() { return NDArray<T>::max(prepareMat()); }
  /**
   ...
   */
  Series<T> min() { return NDArray<T>::min(prepareMat()); }
  /**
   ...
   */
  Series<T> sum() { return NDArray<T>::sum(prepareMat()); }
  /**
   ...
   */
  Series<T> mean() { return NDArray<T>::mean(prepareMat()); }
  /**
   ...
   */
  Series<T> std() { return NDArray<T>::std(prepareMat()); }
  /**
   ...
   */
  Series<T> var() { return NDArray<T>::var(prepareMat()); }
};