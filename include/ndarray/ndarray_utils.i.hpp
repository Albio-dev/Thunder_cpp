...
NDArray<T> NDArray<T>::getPosition(std::vector<uint16_t> pos) {
  ...
  // Starting from the missing dimension calculates the size of the identified
  // matrix
  int endindex = 1;
 ...
 */
template <class T> int NDArray<T>::get_current_dimension() {
 ...
 */
template <class T> T *NDArray<T>::toarray() {
 ...
 */
template <class T> int NDArray<T>::count() {
  if (shape.size() == 1)
    return shape[0];
  //            reshape({1, shape[0]});
...
template <class T> int NDArray<T>::count(NDArray<T> input) {
  return input.count();
}

// TODO: To be protected, internal function
template <class T> NDArray<T> NDArray<T>::transpose(NDArray<T> input) {
...
template <class T> void NDArray<T>::reshape(std::vector<uint16_t> new_shape) {
  shape = new_shape;
}