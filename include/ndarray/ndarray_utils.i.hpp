 ...
 */
template <class T> T *ndarray<T>::toarray() const {
 ...
 */
template <class T> uint16_t ndarray<T>::count() const {
  if (shape.size() == 1)
    return shape[0];
  //            reshape({1, shape[0]});
 ...
 */
template <class T> uint16_t ndarray<T>::count(const ndarray<T> input) const {
  return input.count();
}
 ...
 */
template <class T> ndarray<T> ndarray<T>::transpose(const ndarray<T> input) {
...
/**
 * @brief Change shape to the matrix. new dimensions must add up to the previous
 * ones
 *
