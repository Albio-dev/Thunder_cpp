...
public:
  ...
  template <class K> NDArray<T> operator+(const NDArray<K> other);
  /**
  ...
  // Operator wrapper
  template <class K> NDArray<T> plus(NDArray<K> other);

  template <class K> NDArray<T> minus(NDArray<K> other);
  /**
   * @brief Dot divide a matrix with another. ToDo: Controlli grandezze array e
   * valori siano corretti/compatibibli tipo se un valore é di tipo int e
   * l'altro tipo uint cast di che tipo (int imo)
   *
   * @return
   */
  template <class K> NDArray<T> dotdivide(const NDArray<K> other);
   ...
   */
  template <class K> NDArray<T> dottimes(const NDArray<K> other);
  ...
  /**
   * @brief Check if shape is correct or plausibile and return number of
   * elements
   *
  ...
  /**
   * @brief Applies a function which should evaluate to boolean, along the first
   * axis
   *
   ...
   * If matrix is monodimensional returns a single max value, if there is more
   * than a dimension, it cycles through the first returning the maximum along
   ...
   * If matrix is monodimensional returns a single min value, if there is more
   * than a dimension, it cycles through the first returning the minimum along
   * all other dimensions.
   *
   ...
   * If matrix is monodimensional returns a single sum value, if there is more
   * than a dimension, it cycles through the first returning the sum along all
   * other dimensions.
   *
   ...
   * If matrix is monodimensional returns a single mean value, if there is more
   * than a dimension, it cycles through the first returning the mean along all
   * other dimensions.
   *
   ...
   * If matrix is monodimensional returns a single standard deviation value, if
   * there is more than a dimension, it cycles through the first returning the
   * standard deviation along all other dimensions.
   *
   * @return NDArray<T> Structure containing the vector of standard deviation
   * value(s)
   */
   ...
   * If matrix is monodimensional returns a single variance value, if there is
   * more than a dimension, it cycles through the first returning the variance
